...
Migrator.prototype.migrate = function (done) {
    ...
    self._getAvailableMigrations(function (migrations) {
            ...
            if (!atLeastOneNew) {
                self._logInfo('All migrations up to date');
                done(true);
                return;
            }

            self._logInfo('Starting migrations...');
...
Migrator.prototype._migrationDone = function (migration, err) {
    ...
    if (err) {
        self._logError("Error occured: " + err);
        self.counterFailed++;
        result.success = false;
        result.error = err;
    }
    self._logInfo(result._id + ' ' + (result.success ? 'SUCCESS' : 'FAILED'));
    self.db.collection(self.migrationCollection).updateOne({_id: result._id}, {$set: result}, {upsert: true}, function (err) {
        if (err) {
            self._logError(err);
        }
